...
class Envira_Gallery_Shortcode {
    ...
    public function shortcode( $atts ) {
        ...
        $gallery .= '<div id="envira-gallery-wrap-' . sanitize_html_class( $data['id'] ) . '" class="' . $this->get_gallery_classes( $data ) . '" itemscope itemtype="http://schema.org/ImageGallery">';
            ...
            if ( $this->get_config( 'columns', $data ) > 0 ) {
                ...
                if ( !$mobile ) {
                    $row_height = $this->get_config( 'justified_row_height', $data ) ? $this->get_config( 'justified_row_height', $data ) : 150;
                } else {
                    $row_height = $this->get_config( 'mobile_justified_row_height', $data ) ? $this->get_config( 'mobile_justified_row_height', $data ) : 80;
                }

                $justified_gallery_theme = $this->get_config( 'justified_gallery_theme', $data );
                $justified_margins = $this->get_config( 'justified_margins', $data ) ? $this->get_config( 'justified_margins', $data ) : '0';

                // this is a justified layout, no isotope even if it's selected in the DB
                $isotope = false;
            }

            $gallery .= '<div' .  $opacity_insert . ' data-row-height="'.$row_height.'" data-justified-margins="'.$justified_margins.'" data-gallery-theme="'.$justified_gallery_theme.'" id="envira-gallery-' . sanitize_html_class( $data['id'] ) . '" class="envira-gallery-public '.$extra_css.' envira-gallery-' . sanitize_html_class( $this->get_config( 'columns', $data ) ) . '-columns envira-clear' . $isotope . ( $this->get_config( 'css_animations', $data ) ? ' envira-gallery-css-animations' : '' ) . '" data-envira-columns="' . $this->get_config( 'columns', $data ) . '">';
        ...
        $no_js   .= '</noscript>';
        $gallery .= apply_filters( 'envira_gallery_output_noscript', $no_js, $data );
    ...
    */
    public function generate_gallery_item_markup( $gallery, $data, $item, $id, $i ) {
        ...
        $output   = '<div id="envira-gallery-item-' . sanitize_html_class( $id ) . '" class="' . $this->get_gallery_item_classes( $item, $i, $data ) . '" style="padding-left: ' . $padding . 'px; padding-bottom: ' . $this->get_config( 'margin', $data ) . 'px; padding-right: ' . $padding . 'px;" ' . apply_filters( 'envira_gallery_output_item_attr', '', $id, $item, $data, $i ) . ' itemscope itemtype="http://schema.org/ImageObject">';
            ...
            if ( $create_link ) {
                $mobile_thumb = ! empty( $item['mobile_thumb'] ) ? esc_url( $item['mobile_thumb'] ) : '';
                $output .= '<a ' . $target . ' href="' . esc_url( $item['link'] ) . '" class="envira-gallery-' . sanitize_html_class( $data['id'] ) . ' envira-gallery-link" ' . $html5_attribute . '="enviragallery' . sanitize_html_class( $data['id'] ) . '" title="' . strip_tags( esc_attr( $item['title'] ) ) . '" data-envira-caption="' . $caption . '" data-envira-retina="' . ( isset( $item['lightbox_retina_image'] ) ? $item['lightbox_retina_image'] : '' ) . '" data-thumbnail="' . esc_url( $item['thumb'] ) . '" data-mobile-thumbnail="' . $mobile_thumb . '"' . ( ( isset($item['link_new_window']) && $item['link_new_window'] == 1 ) ? ' target="_blank"' : '' ) . ' ' . apply_filters( 'envira_gallery_output_link_attr', '', $id, $item, $data, $i ) . ' itemprop="contentUrl">';
            }
                ...
                $output_item = '<img id="envira-gallery-image-' . sanitize_html_class( $id ) . '" class="envira-gallery-image envira-gallery-image-' . $i . $gallery_theme . '" data-envira-index="' . $i . '" src="' . esc_url( $imagesrc ) . '"' . ( $this->get_config( 'dimensions', $data ) ? ' width="' . $this->get_config( 'crop_width', $data ) . '" height="' . $this->get_config( 'crop_height', $data ) . '"' : '' ) . ' data-envira-src="' . esc_url( $imagesrc ) . '" data-envira-gallery-id="' . $data['id'] . '" data-envira-item-id="' . $id . '" data-envira-caption="' . $caption . '" alt="' . esc_attr( $item['alt'] ) . '" title="' . strip_tags( esc_attr( $item['title'] ) ) . '" ' . apply_filters( 'envira_gallery_output_image_attr', '', $id, $item, $data, $i ) . ' itemprop="thumbnailUrl" srcset="' . esc_url( $image_src_retina ) . ' 2x" />';
    ...
    */
                            ...
                            afterShow: function(){
                                ...
                                if ( $this->get_config( 'mobile_touchwipe', $data ) ) {
                                    ...
                                    if ( $('#envirabox-thumbs ul li').length > 0 ) {
                                                                            ...
                                                                            console.log ( $('#envirabox-thumbs ul li').length  );
    ...
    public function envirabox_margin( $margin, $data ) {
        ...
        if ( $instance->get_config( 'supersize', $data ) ) {
            $margin = 0;
        }

        if ( $instance->get_config( 'arrows_position', $data ) == 'outside' ) {
            $margin = $margin + 50;
        }
    ...
    public function envirabox_actions( $template, $data ) {
        ...
        $instance = Envira_Gallery_Shortcode::get_instance();
        if ( ! in_array( $instance->get_config( 'lightbox_theme', $data ), array( 'base_dark' ) ) ) {
            return $template;
        }
    ...
    public function envira_supersize_wrap_css_class( $css_classes, $data ) {
        ...
        if ( ! isset( $data['config']['supersize'] ) ) {
            return $css_classes;
        }
